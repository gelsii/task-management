...
type LoginFourProps = {
  username: React.Dispatch<React.SetStateAction<string>> | "none";
  email: React.Dispatch<React.SetStateAction<string>>;
  password: React.Dispatch<React.SetStateAction<string>>;
  onSubmitFN: SubmitEventHandler<HTMLFormElement>;
  isHaveResetPassword: boolean;
  disabledSubmitButton: boolean;
  isHaveUsernameInput: boolean;
  customize: {
    order_ui: Array<number>;
    headingImageBranding: string;
    descriptionImageBranding: string;
    headingForm: string;
    descriptionForm: string;
    textSubmitButton: string;
    lastDescriptionForm: Array<string>;
  };
};

const Login4 = (props: LoginFourProps) => {
  ...
  return (
    <div className="min-h-screen w-full bg-white dark:bg-black flex">
      ...
      <div
        className={`hidden lg:flex lg:w-1/2 order-${props.customize.order_ui[1]}  bg-linear-to-br from-indigo-500 via-purple-600  dark:from-[#0f23fa] dark:via-[#0f85fa] dark:to-[#0fe7fa] relative overflow-hidden`}
      >
        ...
        <div className="relative z-10 flex flex-col justify-center items-center w-full text-white p-12">
          ...
          <h1 className="text-4xl font-bold mb-4 text-center flex gap-2 items-center">
            {props.customize.headingImageBranding}
          </h1>
          <p className="text-sm text-center text-white/90 max-w-md">
            {props.customize.descriptionImageBranding}
          </p>
      ...
      <div
        className={`w-full lg:w-1/2 flex order-${props.customize.order_ui[0]} items-center justify-center p-8 `}
      >
        <div className="w-full max-w-md space-y-8 ">
          {/* Header */}
          <div className="text-center">
            <div className="w-12 h-12 bg-linear-to-r from-indigo-600 dark:from-[#0f23fa] dark:via-[#0f85fa] dark:to-[#0fe7fa] rounded-xl flex items-center justify-center mx-auto mb-6">
              <Shield />
            </div>
            <h2 className="text-3xl font-bold text-gray-900 dark:text-white">
              {props.customize.headingForm}
            </h2>
            <p className="mt-2 text-gray-600 dark:text-gray-400">
              {props.customize.descriptionForm}
            </p>
          </div>
          ...
          {/* Form */}
          <form className="space-y-6" onSubmit={props.onSubmitFN}>
            {props.isHaveUsernameInput && (
              <div>
                <label
                  htmlFor="username"
            ...
            >
              {props.disabledSubmitButton
                ? "Loading..."
                : props.customize.textSubmitButton}
            </button>
          </form>
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {props.customize.lastDescriptionForm[0]}
              <Link
                to={props.customize.lastDescriptionForm[2]}
                className="font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 transition-colors"
              >
                {props.customize.lastDescriptionForm[1]}
              </Link>
            </p>
          </div>
        </div>
